...
export function computeHealth(
  ...
  const noFm = skills.filter((s) => !s.frontmatter || Object.keys(s.frontmatter).length === 0)
  if (noFm.length > 0) {
    diagnostics.push({
      type: 'missing_frontmatter',
      severity: 'danger',
      title: `${noFm.length} 个 Skill 缺失 YAML Frontmatter 标头`,
      detail: `IDE（特别是 Claude Code）要求 SKILL.md 顶部必须包含合法的 YAML Frontmatter (例如 name 和 description)。缺失标头将导致 IDE 无法识别或加载此 Skill。建议一键修复。`,
      affectedSkillIds: noFm.map((s) => s.id),
    })
  }
  ...
  // Missing frontmatter penalty: -5 per affected skill
  score -= noFm.length * 5
