...
function vaultRoot(): string {
  ...
  return path.join(home, '.local', 'share', 'skill-studio', 'vault')
}
...
export async function ensureVault(config: SyncConfig): Promise<{ ok: true; status: VaultStatus } | { ok: false; error: string }> {
  ...
  if (emptyRepoHint || clone.code !== 0) {
    ...
    await runGit(['config', 'user.name', 'Skill Studio'], { cwd: vaultPath })
    await runGit(['config', 'user.email', 'skill-studio@localhost'], { cwd: vaultPath })
